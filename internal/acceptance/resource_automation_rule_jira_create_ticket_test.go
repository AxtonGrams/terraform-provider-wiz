...
func TestAccResourceWizAutomationRuleJiraCreateTicket_basic(t *testing.T) {
	rName := acctest.RandomWithPrefix(ResourcePrefix)

	resource.UnitTest(t, resource.TestCase{
		PreCheck:          func() { testAccPreCheck(t, TestCase(TcJira)) },
		ProviderFactories: providerFactories,
		Steps: []resource.TestStep{
			{
				Config: testResourceWizAutomationRuleJiraCreateTicketBasic(rName),
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(
						"wiz_automation_rule_jira_create_ticket.foo",
						"name",
						rName,
					),
					resource.TestCheckResourceAttr(
...
func testResourceWizAutomationRuleJiraCreateTicketBasic(rName string) string {
	return fmt.Sprintf(`
...
resource "wiz_automation_rule_jira_create_ticket" "foo" {
  name           = "%s"
